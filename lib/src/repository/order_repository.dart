...
Future<Stream<Order>> getOrders({List<String> statusesIds}) async {
  ...
  _queryParams['with'] =
      'productOrders;productOrders.product;productOrders.options;orderStatus;deliveryAddress;payment';
...
Future<Stream<Order>> getOrder(orderId) async {
  ...
  _queryParams['api_token'] = _user.apiToken;
  _queryParams['with'] =
      'driver;user;productOrders;productOrders.product;productOrders.options;orderStatus;deliveryAddress;payment';
  uri = uri.replace(queryParameters: _queryParams);
  print(CustomTrace(StackTrace.current, message: uri.toString()).toString());
  try {
    final client = new http.Client();
